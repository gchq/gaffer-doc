...
These suffixes can be customised which allows for graphs to share the same cache entries if desired. This only applies if the same cache instance is configured to be used by all the graphs instead to share these.
